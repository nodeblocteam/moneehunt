...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab }) => {
  const { isConnected, publicKey, xlmBalance, isConnecting, connectWallet, disconnectWallet } = useStellarWallet();
  ...
  return (
    <nav style={{
      ...
      <div style={{
        ...
        {/* Brand Logo */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', cursor: 'pointer' }} onClick={() => setActiveTab('milestone')}>
          <img src="/assets/logo.svg" alt="MoneeHunt" style={{ width: '42px', height: '42px' }} onError={(e) => {
            // Fallback SVG if relative path varies
            (e.target as HTMLElement).setAttribute('src', 'https://raw.githubusercontent.com/nodeblocteam/moneehunt/main/assets/logo.svg');
          }} />
          <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          {isConnected && publicKey ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <button className="btn btn-secondary" style={{ padding: '8px 14px', fontSize: '0.8rem' }} onClick={disconnectWallet}>
                Disconnect
              </button>
            </div>
          ) : (
